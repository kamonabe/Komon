...
2. src/{{project.name|lower}}/__init__.pyを更新してください：
...
5. 4つのファイル全てを更新したことを確認してください
   - [ ] version.txt
   - [ ] src/{{project.name|lower}}/__init__.py
   - [ ] {{changelog.location}}
...
- [ ] 4つのファイルの更新を**順番に**リマインドした
  - [ ] 1. version.txt
  - [ ] 2. src/{{project.name|lower}}/__init__.py
...
4. **公開確認**
   ```bash
   pip install {{project.name|lower}}=={{version}}
   {{project.name|lower}} --version
   ```
...
- プロパティベーステスト（{{testing.property_framework}}使用）
- 統合テスト
- ユニットテスト
{% else %}
- 統合テスト
...
{{project.name}}では、**3層テスト構造**を採用しています：
{% if testing.property_testing %}
1. **プロパティベーステスト**: 仕様の正確性を数学的に検証
2. **統合テスト**: 複数モジュールの連携を検証
3. **ユニットテスト**: 個別関数のロジックを検証
{% else %}
1. **統合テスト**: 複数モジュールの連携を検証
2. **ユニットテスト**: 個別関数のロジックを検証
{% endif %}
...
- **{{git.main_branch}}ブランチ**: ドキュメント・Spec作成のみ
