...
- `komon monitor`コマンドの引数を変更（--interval → --check-interval）
- Python 3.9のサポートを終了（最低バージョン: 3.10+）
```
...
| 変更内容 | バージョン | 理由 |
|---------|-----------|------|
| 表示件数のデフォルト変更 | **PATCH** | ユーザー体験の小改善、オプションで従来の動作も可能 |
| 新しいオプション追加（`--history N`） | **MINOR** | 機能拡張（既存動作は変わらない） |
| 既存オプションの削除 | **MAJOR** | 破壊的変更 |
| 既存オプションの非推奨化（deprecation） | **MINOR** | まだ動作する、警告を表示 |
| 内部リファクタリング | **PATCH** | ユーザーには見えない |
| テストの追加のみ | **なし** | 開発者向け改善 |
| CI/CDの導入 | **なし** | 開発者向け改善 |
| Specテンプレートの追加 | **なし** | 開発者向け改善 |
| README.mdの大幅な改善 | **PATCH** | ドキュメント修正 |
| 新しいドキュメントの追加（SECURITY.md等） | **PATCH** | ドキュメント追加 |
| エラーメッセージの改善 | **PATCH** | ユーザー体験の改善 |
| ログ出力の追加 | **PATCH** | デバッグ改善 |
| パフォーマンス改善（2倍高速化） | **PATCH** | 機能追加ではない |
| 新しい依存パッケージの追加 | **MINOR** | 環境への影響がある |
| 依存パッケージの削除 | **MAJOR** | 破壊的変更の可能性 |
| バグ修正 + テスト追加 | **PATCH** | テスト追加は開発者向け改善 |
| バグ修正 + ステアリングルール拡充 | **PATCH** | ルール拡充は開発者向け改善 |
...
`docs/CHANGELOG.md`の`[Unreleased]`セクションを確認します。
