...
## 🚨 Kiroへの厳格な指示（push前チェック）

**GitHubにpushする前に、Kiroは以下を必ず実行：**

### フェーズ1: push前の宣言
```
GitHubにpushする前に品質チェックを実行します。
```
...
**🚨 必須チェック項目（全て完了してからpush）**:

**品質チェック**:
- [ ] `python3 scripts/check_status_consistency.py` を実行した
- [ ] チェック結果が成功（Exit Code: 0）だった
- [ ] エラーがある場合は修正した
