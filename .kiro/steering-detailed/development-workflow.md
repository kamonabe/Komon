...
実装開始前に、Kiroは `git-workflow.md` に定義された**5フェーズ安全チェック**を厳格に実行します。

**概要**:
1. 環境確認（`git branch`, `git status`, `date`）
2. mainブランチにいる場合は即座に停止、開発ブランチ作成を依頼
3. ユーザーの「ブランチ作成完了」返答後に再確認
4. 開発ブランチ確認後、実装開始

**詳細な手順・チェックリスト・エラー回復手順は `git-workflow.md` を参照してください。**
...
push前に `git-workflow.md` に定義された品質チェックを実行します。
...
エラーがある場合は修正してから再チェック。全てパスしてからpush。

**詳細なフェーズ分け・チェックリストは `git-workflow.md` を参照してください。**
...
ブランチ戦略、ブランチ命名規則、安全策、マージテスト、トラブルシューティングの詳細は全て `git-workflow.md` を参照してください。
