...
- **段階的閾値**: 3段階（警告/警戒/緊急）の閾値設定
- **警告レベル**: 最初の注意喚起レベル（デフォルト70%）
- **警戒レベル**: 中程度の警告レベル（デフォルト80%）
- **緊急レベル**: 最も深刻な警告レベル（デフォルト90%）
- **ThresholdLevel**: 閾値レベルを表す列挙型（NORMAL/WARNING/ALERT/CRITICAL）
- **後方互換性**: 既存の単一閾値設定をサポートすること
- **エスカレーション**: 閾値レベルが上昇すること（例: 警告→警戒）
