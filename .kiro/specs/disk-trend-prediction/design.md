...
**検証方法**: ランダムな時系列データを生成し、日次平均が正しく計算されることを確認
...
**検証方法**: 欠損を含むデータセットを生成し、予測が実行されることを確認
...
**検証方法**: 既知の傾きを持つデータを生成し、計算結果が期待値と一致することを確認（許容誤差: 0.01）
...
**検証方法**: ランダムな増加傾向データを生成し、予測日の計算が正しいことを確認
...
**検証方法**: ランダムなデータを生成し、前日比が正しく計算されることを確認
...
**検証方法**: 様々な前日比を持つデータを生成し、閾値判定が正しいことを確認

**検証要件**: 3.2, 3.3

### プロパティ7: 予測メッセージの完全性

*任意の*予測結果に対して、生成されるメッセージは以下の要素を含まなければならない：
- 増加トレンドの説明（傾きまたはトレンド）
- 90%到達予測日が存在する場合は「あとN日で90%に到達」
- 急激な変化が検出された場合は「急激な増加」の警告
- 警告がある場合は推奨アクション

**検証方法**: 様々な予測結果を生成し、メッセージに必要な要素が含まれることを確認
...
**検証方法**: 複数の警告を持つデータを生成し、メッセージの順序が正しいことを確認
