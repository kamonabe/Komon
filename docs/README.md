...
| ✅ 通知履歴機能                          | v1.11.0で追加        | ローカルファイルに通知履歴を自動保存 |
| ✅ 週次健全性レポート                    | v1.12.0で追加        | 毎週定期的にシステム状態をSlack/メールで送信 |
| ✅ 3段階閾値通知システム                 | v1.13.0で追加        | 警告/警戒/緊急の3段階で段階的に通知 |
| ✅ 環境変数対応の強化                    | v1.14.0で追加        | 認証情報を環境変数で管理（セキュリティ向上） |
| ✅ 通知頻度制御機能                      | v1.15.0で追加        | 同一アラートの通知を抑制、通知疲れを防止 |
| ✅ ディスク使用量の増加トレンド予測       | v1.16.0で追加        | 過去7日分のデータから線形回帰により将来の使用量を予測 |
| ✅ 段階的通知メッセージ機能              | v1.17.0で追加        | 同じ問題が繰り返し発生した場合、メッセージが段階的に変化 |
| ✅ コンテキスト型アドバイス機能          | v1.18.0で追加        | プロセスの種類に応じた具体的なアドバイスを提供 |
| ✅ Slack通知にプロセス情報を含める       | v1.19.0で追加        | CPU/メモリ超過時に上位3プロセスの情報を自動追加 |
| ✅ 多重実行プロセスの検出機能            | v1.20.0で追加        | 同一スクリプトの多重起動を検出し警告 |
| ✅ ログ急増時の末尾抜粋表示機能          | v1.21.0で追加        | ログ急増検出時に末尾N行を自動抽出して表示 |
| ✅ 長時間実行プロセスの検出機能          | v1.22.0で追加        | 特定スクリプトが長時間実行されている場合に検出 |
| ✅ 出力フォーマットの改善                | v1.23.0で追加        | システム状態を最初に表示、ノイズ削減、視覚的改善 |
| ✅ OS判定・汎用Linux対応                 | v1.24.0で追加        | 実行環境のOSを自動判定し適切なアドバイスを提供 |
| ✅ ネットワーク疎通チェック機能          | v1.25.0で追加        | Ping/HTTPチェックで外部サービスへの疎通を確認 |
