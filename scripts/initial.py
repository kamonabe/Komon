...
def run_initial_setup():
    ...
    sample_path = "config/settings.yml.sample"
    if not os.path.exists(sample_path):
        print(f"❌ {sample_path} が見つかりません。")
        print("   リポジトリのルートディレクトリで実行してください。")
        return

    with open(sample_path, "r", encoding="utf-8") as f:
        settings = yaml.safe_load(f)

    print("📋 config/settings.yml.sample から設定を読み込みました。")
    ...
    print("✅ settings.yml を作成しました！\n")
    print("🎯 次のステップ：")
    print("  → komon advise を実行してみましょう！")
    print("  → cron登録もおすすめです。\n")
    print("📁 補足：")
    print("  詳細な設定は settings.yml を直接編集してください。")
